...
int main() {
  ...
  if (test_1) {
    ...
    std::ofstream output;
    output.open("deterministic_paths.csv");
    ...
    for (int i = 0; i < MAX_TEST_ITERATIONS; i++) {
      ...
      for (size_t i = 0; i < learned_path.size(); i++) {
        output << learned_path[i] << ",";
      }
      output << std::endl;
    }
  }
    ...
    for (; epsilon_value <= 100.0f; epsilon_value = epsilon_value + 1.0f) {
      for (; alpha <= 100.0f; alpha = alpha + 1.0f) {
        ...
        for (int j = 0; j < iterations; j++) {
          ...
          if (j == iterations - 1) { /*Disaster condition*/
  ...
  if (test_4) {
    ...
    int iterations = 10000;
    float epsilon_value = 50.0f;
    float alpha = 0.1f;
    std::ofstream output1;
    output1.open("decaying_epsilon_random_marbles.csv");

    epsilon_value = 50.0f;
    for (int i = 0; i < 100; i++) {
      float epsilon_decay_rate = ((float)i / 100);
      q_learning q_learner(test_map, marble_map, epsilon_value / 100);
      ...
      for (int j = 0; j < iterations; j++) {
        ...
        while (!S.is_outside_environment) {
          ...
          S = S_next;
        } /*run iterations end, when S is terminal*/
        //        std::cout << "DATA: " << epsilon_value / 100 << std::endl;
        //        output1 << epsilon_value / 100 << "," << j << std::endl;
        ...
        if (q_learning::is_vector_comparable(learned_path, expected_pathing)) {
          std::cout << "DATA: " << epsilon_decay_rate << "," << j << std::endl;
          output1 << epsilon_decay_rate << "," << j << std::endl;

          break;
        }
        //        epsilon_value -= epsilon_value * epsilon_decay_rate;
        q_learner.epsilon_value -= epsilon_decay_rate / 1000;
      } /*learning end*/
      epsilon_value = 50.0f;
    } /*For epsilon end*/
    output1.close();
    std::cout << "DATA1 CREATED" << std::endl;
  }
  /*****************************************************************************
   * TEST 5:
   * fixed marble, test for deterministic path, alpha = 1
  ...
  if (test_5) {
    ...
    float epsilon_value = 0.1f; /*greedy value*/
    float alpha = 0.4f;         /*step size*/
    int time_to_live = 50;

    q_learning q_learner(room_map, marble_map, epsilon_value, RANDOM_MARBLES);
    /*
     * LEARNING LOOP FOR EACH EPISODE
     * */
    for (int j = 0; j < iterations; j++) {
      ...
      std::vector<int> learned_path =
          q_learner.return_learned_path(time_to_live);
      if (q_learning::is_vector_contained_inside(learned_path, all_rooms)) {
        std::cout << "Learned path:" << std::endl;
        for (size_t n = 0; n < learned_path.size(); n++) {
          std::cout << learned_path[n] << " ";
        }
        std::cout << std::endl;
        break;
      }
